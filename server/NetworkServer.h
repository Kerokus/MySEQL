...
private:
	...
	// EQL adaptation: when non-zero, SpawnHeaderAddr points to an entity-list MANAGER whose
	// [+spawnListHop] holds the first spawn (two-hop head). We then walk forward-only via
	// NextOffset and skip the prev-walk (the manager's +hop is already the true list head).
	// Zero (default / live EQ) = SpawnHeaderAddr points directly at a spawn, prev-walk as usual.
	QWORD spawnListHop;
