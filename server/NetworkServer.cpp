...
void NetworkServer::init(IniReaderInterface* ir_intf)
{
	...
	// EQL: optional manager-hop. 0 (or key absent) = classic single-deref behavior.
	spawnListHop = ir_intf->readIntegerEntry("Memory Offsets", "SpawnListHop");
...
void NetworkServer::handleSpawnsRequest(MemReaderInterface* mr_intf)
{
	...
	if (spawnListHop)
	{
		// EQL: SpawnHeaderAddr -> entity-list manager; the first spawn is [manager + spawnListHop].
		// That head is authoritative (no shroud/hover mid-list entry), so we skip the prev-walk and
		// let the forward walk below enumerate via NextOffset until next == 0.
		pTemp = pTemp2 = mr_intf->extractPointer(pTemp + spawnListHop);
	}
	else
	{
...
void NetworkServer::updateGUI(QWORD& pTemp, MemReaderInterface* mr_intf, int& maxLoop)
{
	if (loopCount > 20) {
		...
		if (spawnListHop)
		{
			// EQL: hop from manager to the first spawn; head is authoritative (no prev-walk).
			pTemp = mr_intf->extractPointer(pTemp + spawnListHop);
		}
		else
		{
